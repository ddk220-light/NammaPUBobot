...
- An eligible inactive holder loses `available_balance // 10`, computed under the wallet lock when the assessment runs. The ledger records the negative amount as `inactivity_tax`; it is not added to a betting pot.
- Tax cannot make a balance negative. Whole-gold rounding means balances below 10 incur no deduction. For example, 10,000 idle gold becomes 6,561 after four consecutive weekly assessments, assuming no other movements.
