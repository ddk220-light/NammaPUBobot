...
matches, civilization results, a play-money gold economy with betting on live
matches, a daily quiz, and a public web dashboard.
...
- **Lightweight match tracking.** Match wins/losses, rating changes, the bot
  match ID ↔ AoE2 match ID link, and civilization outcomes are recorded without
  downloading a replay. Replay download/parsing, scouting reports, Match Cards,
  and rendered Elo charts are currently paused; historical replay-derived quiz
  and leaderboard values remain readable but are not refreshed.
...
- **A public web dashboard** — leaderboards, player pages and civ stats — plus
  an authenticated config surface for admins. Historical play-style data may
  still appear on legacy pages but is frozen while replay analysis is paused.
  ...
  community. Replay compute is off by default, and hosted deployments hard-
  disable it; a self-hosted operator must explicitly enable it globally and per
  community. Authenticated settings use
...
The default install is the lightweight product and does not include replay
parsing or Matplotlib. A self-hosted operator intentionally re-enabling those
legacy features installs `requirements-replay.txt` instead and builds the
Docker image with `--build-arg INSTALL_REPLAY_DEPS=true`.
...
[RAILWAY_SETUP.md](RAILWAY_SETUP.md). The cost-sensitive production settings,
backup/purge interlocks and MySQL canary procedure are in
[docs/runbooks/railway-low-cost-mysql.md](docs/runbooks/railway-low-cost-mysql.md).
