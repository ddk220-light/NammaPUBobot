...
def test_series_early_elim_mean_reflects_zero_fill():
    """Mean of early-eliminated player includes the zero-filled tail."""
    s = apm_series(ROWS_WITH_EARLY_ELIM, NAMES_WITH_EARLY_ELIM)
    charlie = next(x for x in s if x["player_number"] == 3)

    # Charlie's mean is (10 + 25 + 0 + 0) / 4 = 8.75
    assert charlie["mean"] == 8.75, (
        "Mean should include zero-filled values from after elimination"
    )
