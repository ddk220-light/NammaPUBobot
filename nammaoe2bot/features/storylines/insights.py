...
def subject_of(c):
	...
	resolves the player to a side. nammaoe2bot/features/storylines/payoff.py depends on this.
	"""
	t, d = c["type"], c["data"]
	if t == "h2h":
...
def complement_of(c, nick, teams_meta, rosters):
	...
	single subject side). Public because nammaoe2bot/features/storylines/payoff.py builds a
...
def _ordered_pair(c):
	...
	Shared by ``rival_backers`` (below) and nammaoe2bot/features/storylines/payoff.py's
...
def rival_backers(c, nick, teams_meta, rosters):
	...
	because nammaoe2bot/features/storylines/payoff.py builds a past-tense version from the same data.
	"""
	out = []
