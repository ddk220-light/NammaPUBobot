...
stage 5b (`nammaoe2bot/features/quiz/player_bank.py` builds the player questions live from
...
constraint as nammaoe2bot/community.py.
"""
import time
...
# happens, and importing nammaoe2bot.features.identity.resolver from inside it would execute the
...
# it cannot import nammaoe2bot.features.identity.resolver (see this module's docstring, and
...
async def record_refused_claim(profile_id, claimed_user_id, source) -> None:
	...
	nammaoe2bot/features/identity/solver.py is the caller: it refuses to auto-apply a conclusion
...
# `community_channels` is nammaoe2bot/community.py's; this module only ever READS
