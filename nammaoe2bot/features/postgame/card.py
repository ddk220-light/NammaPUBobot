...
combat outcomes. The win/loss storylines live elsewhere: nammaoe2bot/features/storylines/insights.py
teases them when teams are formed, and nammaoe2bot/features/storylines/payoff.py settles them at
report time.
...
async def post_match_analysis(bot_match_id):
	...
	try:
		...
		# nammaoe2bot/features/storylines/payoff.py, so only the cards are left here.
		rows = await _analysis_rows(bot_match_id)
		team_names = await _team_names(channel_id, bot_match_id)
