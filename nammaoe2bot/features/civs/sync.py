...
# nammaoe2bot/features/civs/pools.get_today_civs to drive /test_random_civs' "exclude civs played
...
db.ensure_table(dict(
	...
	columns=[
		...
		# Linkage to the bot match + player, filled by nammaoe2bot/features/civs/matcher.py when a
...
async def record_lobby_match(channel_id, bot_match_id, players, winner, match_at, parsed,
	...
	if uid_to_pids is None or nick_to_pids is None:
		# No nick fallback here either (see nammaoe2bot/features/civs/matcher._map_players_to_profiles) —
...
def link_and_write(bot_match_id, lobby_data):
	...
	`civ_picks` table (persist_lobby_civs + nammaoe2bot/features/civs/matcher.py). Retiring this
