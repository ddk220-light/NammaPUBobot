...
def extract_match(path, date_map=None):
    ...
    # noqa rather than a `with`: this file was linted for the first time when it
    # moved out of the ruff-excluded utils/replay_quiz/, and closing the handle
    # here is a behaviour change to the live parser (mgz.model.parse_match is
    # handed the stream, and whether it holds a reference past return is its
    # business, not ours to assume mid-move). Worth fixing on its own, with its
    # own verification against a real replay.
    m = mgz.model.parse_match(open(path, "rb"))   # noqa: SIM115
    ...
    for pnum, p in players.items():
        ...
        for (_bn, building), n in [((k[0], k[1]), v) for k, v in builds.items() if k[0] == pnum]:
            out_buildings.append(dict(player_number=pnum, identity=identity, civ=civ,
                                      building=building, count=n))

        for (_tn, tech), click in [((k[0], k[1]), v) for k, v in research_first.items() if k[0] == pnum]:
            phase = "dark"
            if ic and click >= ic:
                phase = "imperial"
