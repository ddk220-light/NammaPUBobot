...
- **`bot/predictions/`** — Audience predictions on ranked matches, now pari-mutuel gold betting rather than free votes. `bot/predictions/gold.py` is the ONLY module that writes `gold_ledger` (append-only), `gold_balances` (the spendable cache) and `prediction_bets` — every movement, seed through payout, is one `db.transaction()`. Every seed/reward/refund/payout row is an idempotent `INSERT IGNORE` keyed on `idem_key` (double-application is a no-op, not a double-pay); `bet` and `cancel` rows carry `idem_key = NULL`, since both are deliberately repeatable — the side lock and the `DELETE` rowcount, not idem-keying, are what make them safe. Players get a match-reward faucet of `min(10, max(0, 500 − balance))` gold on every completed ranked match (never lifts a balance above the 500 ceiling); everyone gets a one-time 500-gold seed, applied idempotently in bulk on boot and lazily on a bettor's first press. Match participants may bet too, but **only on their own team** — a press on the opposing side is refused, which is what keeps "no participant can profit by losing" true; spectators are unrestricted, either side. `prediction_bets.is_player` is captured **at press time**, not derived later, because the roster lives in the in-memory `bot.active_matches` and is gone by report time. `Match.teams` has **three** entries — `[0]`, `[1]`, and `[2]` = the pseudo-team `"unpicked"` (`idx=-1`) — so the own-team check indexes `[0]`/`[1]` explicitly and must never iterate `match.teams`. A user's first press locks their side for that post via the `prediction_bets` composite PK; further presses on that side add to the stake, a press on the other side is rejected. A bettor may cancel while the post is still `open` and `now < freezes_at` — after the freeze there are no cancels. Cancel returns the bettor's entire stake, deletes the `prediction_bets` row, and releases the side lock; its exactly-once guard is the `DELETE`'s rowcount, **not** an `idem_key` — a user may bet → cancel → bet → cancel on one post, which makes a per-post key non-unique. If the book is still one-sided when it freezes, the post goes `no_action` and every stake is refunded immediately, because a book with no opposing gold has no odds to settle. A settled book pays `floor(stake_i * total_pool / winning_pool)` per winner; the undistributed remainder is burned, not carried over. `prediction_votes` is now historical-only (no writer) — the accuracy leaderboard `UNION ALL`s its frozen `is_correct` with graded `prediction_bets` so votes-era history still counts. Bet buttons route through `bot/predictions/interactions.py` on the global `on_interaction` chain in `bot/events.py`, the same pattern as the quiz's component router — DB-driven and redeploy-safe, no persistent Views to lose on restart. **Every sweep that settles a book closes it FIRST** and only then snapshots it with `bets_for` — and `gold.place_bet` re-reads that same post row `FOR UPDATE` inside its own transaction. The two halves are one mechanism: a press that commits after a refund/payout snapshot but before the status flips is refunded by nobody and paid by nobody, and `reconcile()` cannot see the loss because the ledger and the balance cache still agree. Never move a `bets_for` above its `_close_book`. **The close is also where the terminal branch is claimed**: `store.claim_terminal(post_id, 'settle'|'void'|'no_action')` writes `prediction_posts.terminal_intent` in the SAME compare-and-set that flips `status` to `'frozen'`, before a single coin moves, and every resume path (`unsettled_books`, `abandoned_books`, `resolve_for_match`, `_settle`) DISPATCHES ON THAT STORED INTENT rather than re-deriving a branch from `matches.winner`. A caller whose claim comes back as somebody else's branch stands down without moving gold. This is not decoration: "money first, terminal status last" is sound only if a crash between them leaves the post retryable *on the same branch*, and for a two-sided book whose match later reported a winner it was not — `live_for_match` returns `status IN ('open','frozen')`, so a void that refunded every stake and died before `status='void'` landed was handed straight back to settlement, which paid `scoring.payouts()` over the very bets it had just refunded. `refund:{post}:{user}` and `payout:{post}:{user}` are different `idem_key`s, so nothing collided and nothing self-healed. `store.close_betting` (the plain compare-and-set on `status='open'`) survives as the NON-terminal close and has exactly one caller, `_freeze`, whose two-sided arm has no branch to stake yet; its one-sided arm claims `'no_action'` after `bets_for` and still before any refund. Terminal status (`void`/`no_action`/`resolved`) is still written last. **A roster change under a live book voids, refunds and re-opens it** — `Draft.sub_auto` AND `Draft.sub_for` (`/subfor`, `/subforce`) both call `restart_for_match`; without it a spectator staked on one team can be substituted onto the other and profit by losing, which is the exact thing the own-team rule exists to prevent. **`PredictionJobs._run` is the only re-runner**: `resolve_for_match`'s single caller (`finish_match`) has already dropped the match from `bot.active_matches`, so a process that dies mid-payout takes settlement with it; the sweep's second loop re-settles `store.unsettled_books` (frozen posts joined to a `matches` row older than `RESUME_AFTER`), which is what makes the idempotent inserts' crash-safety actually happen, and its third refunds `store.abandoned_books` (frozen posts with **no** `matches` row after `ABANDON_AFTER`), the case where the match itself vanished and no event will ever hand the stakes back. For the same reason, settlement that cannot apply its payouts (no community) writes **nothing at all** — no `resolved`, no report — because a betting report over a payout that never moved is a lie the channel cannot check, and `resolved` would put the debt beyond the resume sweep forever
