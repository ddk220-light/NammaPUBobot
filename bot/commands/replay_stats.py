...
__all__ = ["replaystats_status", "replaystats_backfill", "replaystats_reingest"]


async def replaystats_status(ctx):
    from bot.replay_stats import store, PARSER_VERSION
    from core.database import db
    counts = await db.fetchall("SELECT status, COUNT(*) n FROM replay_ingest GROUP BY status")
    done = await db.fetchall("SELECT MAX(parsed_at) m, COUNT(*) n FROM replay_matches")
    pend = await db.fetchall(
        "SELECT save_version, COUNT(*) n FROM replay_ingest WHERE status='pending_parser_update' "
        "GROUP BY save_version")
    enabled = store.is_enabled()
    parts = [f"Replay-stats **{'ON' if enabled else 'OFF'}** (REPLAY_INGEST_ENABLED) · "
